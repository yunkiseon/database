...
--제약조건 : 데이터 무결성 유지를 위해서(오라클에서만 쓰는 것이 아님)
...
--테이블 재 생성- 연습, 제약조건 이름 부여
CREATE TABLE TABLE_NOTNULL(
	LOGIN_ID VARCHAR2(20) CONSTRAINT TBLNN_LOGID_NN NOT NULL,
	LOGIN_PWD VARCHAR2(20) CONSTRAINT TBLNN_LOGPWD_NN NOT NULL,
	TEL VARCHAR2(20)
);
...
--TEL 제약조건 이름 지정 추가
ALTER TABLE TABLE_NOTNULL MODIFY(TEL CONSTRAINT TBLNN_LOGTEL_NN NOT NULL);
--제약조건 이름 변경
ALTER TABLE TABLE_NOTNULL RENAME CONSTRAINT SYS_C008356 TO TBLNN_LOGTEL_NN;
--제약조건 삭제
ALTER TABLE TABLE_NOTNULL DROP CONSTRAINT TBLNN_LOGTEL_NN;

--2. UNIQUE 
--NAME UNIQUE 를 하면 동일한 이름의 사람 등록 불가능해짐 
...
INSERT INTO TABLE_UNIQUE VALUES('test01','test02','010-5678-7891');
--null 값 중복은 됨 이유: unique 가 null을 잡진 않기 때문
INSERT INTO TABLE_UNIQUE VALUES(NULL, 'test02','010-1234-5678');

--update 할 때도 잘 작동한다.
INSERT INTO TABLE_UNIQUE VALUES('test02','test01','010-1234-7896');
...
--unique 제약조건 이름 지정
CREATE TABLE TABLE_UNIQUE(
	LOGIN_ID VARCHAR2(20) CONSTRAINT TBLUN_LOGID_UNI UNIQUE,
	LOGIN_PWD VARCHAR2(20) NOT NULL,
	TEL VARCHAR2(20)
);
...
ALTER TABLE TABLE_UNIQUE MODIFY(TEL CONSTRAINT TBL_LOGTEL_UNI UNIQUE);
ALTER TABLE TABLE_UNIQUE RENAME CONSTRAINT SYS_C008359 TO TBL_LGNPWD_UNQ;

--3. PRIMARY KEY(기본키)
--가장 많이 사용하는 제약조건, UNIQUE + NOT NULL
...
--NOT NULL, 중복 방지 
INSERT INTO TABLE_PK VALUES('test01','test01','010-1234-7896');
INSERT INTO TABLE_PK VALUES(NULL, 'test01','010-1234-7896');

--WHERE PK컬럼 = 1; , PK를 쓰면 INDEX로 잡아주기 때문에 검색 용이하다.
...
--무결성 제약조건(SCOTT.SYS_C008365)이 위배되었습니다- 부모 키가 없습니다
INSERT INTO EMP_FK(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES(7936, 'KONG', 'MANAGER', 7788, '2016-01-02', 4600, NULL, 50);


--입력 시 부모 테이블 데이터 먼저 입력 후 자식테이블 데이터 입력
INSERT INTO DEPT_FK VALUES(10, 'DATEBASE', 'SEOUL');
INSERT INTO EMP_FK(EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO)
VALUES(7936, 'KONG', 'MANAGER', 7788, '2016-01-02', 4600, NULL, 10);
...
--삭제 -> 자식 레코드가 있는 상황이라면 무결성 제약조건 위배되어 부모삭제 불가
--그래서 자식레코드부터 삭제하고 부모레코드삭제해야하함
DELETE FROM EMP_FK WHERE EMPNO = 7936;
...
--제약조건명 + 부모데이터 삭제 시 자식 데이터 처리 방법 지정
--1) ON DELETE CASCADE : 부모 데이터 삭제 시 참조하는 데이터도 함께 삭제
--2) ON DELETE SET NULL : 부모 데이터 삭제 시 참조하는 데이터에 NULL 로 설정
CREATE TABLE DEPT_FK(
...
--에러없이 작동이 된다. 하지만 테이블 DROP 시에는 참조 테이블부터 해야하는 것은 그대로이다. 
DELETE FROM DEPT_FK WHERE DEPTNO = 10;
...
--체크 제약조건이 위배
INSERT INTO TABLE_CHECK VALUES('test01', 'test', 8, '010-123-7869');
--통과
INSERT INTO TABLE_CHECK VALUES('test01', 'test', 12, '010-123-7869');


--DEFALUT : 값을 지정하지 않은 특정한 열에 기본값을 지정
--지정안하면 기본값을 true, false 등으로 지정할 때 사용함

CREATE TABLE TABLE_DEFALUT(
	LOGIN_ID VARCHAR2(20) NOT NULL,
...
--pwd = 123로 출력됨
INSERT INTO TABLE_DEFALUT(LOGIN_ID, TEL) VALUES('test01','010-1234-7869');
SELECT * FROM TABLE_DEFALUT td; 
-- null로 출력됨
INSERT INTO TABLE_DEFALUT VALUES('test02', NULL,'010-1234-7869');
